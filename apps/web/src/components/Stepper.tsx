...
const Step: React.FC<StepProps> = ({ index, step, label, currentStep }) => {
  return (
    <div
      className={`flex flex-col items-start transition ease-in-out ${
        currentStep == index + 1 ? 'scale-105 px-1' : ''
      }`}
    >
      <div aria-hidden className='flex justify-center items-center'>
        ...
        <label
          className={`hidden ml-2 text-base w-min font-bold whitespace-nowrap ${
            currentStep == index + 1 ? 'text-bcBluePrimary !block' : 'text-gray-400 xl:block'
          } `}
        >
          {label}
        </label>
